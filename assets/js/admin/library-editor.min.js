( function( $ ) {
	...
	var TmpcoderElementorEditorPopup = {
		...
		loadPreview: function() {
			window.elementorFrontend.hooks.addAction( 'frontend/element_ready/tmpcoder-offcanvas.default', function( $scope ) {
			...
			window.elementorFrontend.hooks.addAction( 'frontend/element_ready/tmpcoder-advanced-accordion.default', function( $scope ) {
				$scope.find( '.tmpcoder-template-edit-btn' ).on( 'click', TmpcoderElementorEditorPopup.renderPopup );
			} );
		},
	...
	var TmpcoderModalPopups = {
		...
		settingsNotification: function() {
			var version = $('#tmpcoder-addons-editor-js-js').attr('src');
				version = 'v-'+ version.substring(version .length - 6, version .length);

			// Get Close Time
			var closeTime = JSON.parse( localStorage.getItem( 'TmpcoderPopupEditorNotification'+ version) ) || {};
			...
			$( '#tmpcoder-template-settings-notification .eicon-close' ).on( 'click', function() {
				...
				localStorage.setItem( 'TmpcoderPopupEditorNotification'+ version, Date.now() );
			});
		},
	};
	...
	var TmpcoderThemeBuilder = {
		...
		init: function() {
			...
			// Load Preview
			window.elementor.on( 'preview:loaded', TmpcoderThemeBuilder.onPreviewLoad );
		},

		onPreviewLoad: function() {

			// Popup Settings Notification
			TmpcoderThemeBuilder.settingsNotification();
		...
		settingsNotification: function() {
			...
			// Get Close Time
			var closeTime = JSON.parse( localStorage.getItem('TmpcoderThemeBuilderNotification'+ version) ) || {};
			...
			setTimeout(function() {
				...
				if ( !$('#tmpcoder-template-settings-notification').length ) {
					...
					$('#tmpcoder-template-settings-notification .eicon-close').on( 'click', function() {
						...
						// Save Close Time in Browser
						localStorage.setItem( 'TmpcoderThemeBuilderNotification'+ version, Date.now() );
					});
				}
			}, 1000 );
		},
	...
	$( window ).on( 'elementor:init', TmpcoderThemeBuilder.init );
    
    // Sticky Section Editor JS
    $(window).on('elementor:init', function () {
		elementor.settings.page.addChangeCallback( 'tmpcoder_rpb_enable', function ( newValue ) {
            if('yes' == newValue ) {
                let wrpbHtml = '<div class="tmpcoder-reading-progress-bar-container"><div class="tmpcoder-reading-progress-bar tmpcoder-mybar" id="tmpcoder-mybar"></div></div>';
                $( elementorFrontend.elements.$body ).append( wrpbHtml );
            } else {
                if ( $( elementorFrontend.elements.$body ).find( '.tmpcoder-reading-progress-bar-container' ).length ) {
                    $( elementorFrontend.elements.$body ).find( '.tmpcoder-reading-progress-bar-container' ).remove();
                }
            }
        });
	});
	...
	$(window).on('elementor:init', function () {
		// Data Table
		elementor.hooks.addAction( 'panel/open_editor/widget/tmpcoder-data-table', function( panel, model, view ) {
			elementor.channels.editor.on( 'my-table-export',() => {
				
				console.log('table exporting');
				// var html = view.$el.find('.tmpcoder-data-table');
				var html = view.$el.find('.tmpcoder-data-table').prop('outerHTML');
	...
	function htmlToCSV(html, filename, view) {
		var data = [];
		var rows = view.$el.find('.tmpcoder-data-table').find(".tmpcoder-table-row");
...
// 	$('.tmpcoder-facebook-login').on('click',  connectInstagramInit);
